...
foreach (array(1 => '.dropdown', 2 => '.columns2', 3 => '.columns3', 4 => '.columns4') as $i => $class) {
	...
	$css[] = '@media (min-width: 768px) and (max-width: 959px) { #menu .dropdown { width: '.intval($this['config']->get('menu_width')).'px; }}';
}
...
if ($this['config']->get('direction') == 'rtl') {
	...
	if (($background = $this['config']->get('background')) && $this['path']->path("css:/background/$background-rtl.css")) { $this['asset']->addFile('css', "css:/background/$background-rtl.css"); }
}
...
$fonts = array(
	...
	'nobile' => 'template:fonts/nobile.css',
