	...
	for ($i = 0; $i < $this->itemCount; $i++)
	{
        ...
        <tr class="shl-line-wrap350">
          ...
          <td class="shl-list-shurl">
            <?php
			echo $this->slowServer ? '' : ShlHtmlBs_Helper::badge($this->escape($url->pageid), 'info');
			?>
          </td>
		...
		$name = '-editurl-' . $url->id;
		echo ShlHtmlModal_helper::modalLink($name, $this->escape($url->oldurl), $targetUrl, Sh404sefFactory::getPConfig()->windowSizes['editurl']['x'],
			Sh404sefFactory::getPConfig()->windowSizes['editurl']['y'], $top = 0, $left = 0, $onClose = '', $modalTitle, $params);
		// small preview icon
		...
		else
		{
			...
			echo ShlHtmlModal_helper::modalLink($name, ShlHtmlBs_Helper::iconglyph('', 'tags'), $targetUrl,
				Sh404sefFactory::getPConfig()->windowSizes['editurl']['x'], Sh404sefFactory::getPConfig()->windowSizes['editurl']['y'], $top = 0,
		...
		if ($this->slowServer)
		{
			...
			$anchor = ShlHtmlBs_Helper::badge($url->rank == 0 ? '[<strong>+++</strong>]' : '++', 'warning');
			echo ShlHtmlModal_helper::modalLink($name, $anchor, $targetUrl, Sh404sefFactory::getPConfig()->windowSizes['duplicates']['x'],
				Sh404sefFactory::getPConfig()->windowSizes['duplicates']['y'], $top = 0, $left = 0, $onClose = '', $modalTitle, $params);
		}
		...
		else
		{
			...
			$name = '-editurlduplicates-' . $url->id;
			$anchor = ShlHtmlBs_Helper::badge($url->duplicates, 'warning');
			echo ShlHtmlModal_helper::modalLink($name, $anchor, $targetUrl, Sh404sefFactory::getPConfig()->windowSizes['duplicates']['x'],
				Sh404sefFactory::getPConfig()->windowSizes['duplicates']['y'], $top = 0, $left = 0, $onClose = '', $modalTitle, $params);
		}
			?>
		...
		else
		{
			...
			echo ShlHtmlModal_helper::modalLink($name, $anchor, $targetUrl, Sh404sefFactory::getPConfig()->windowSizes['editurl']['x'],
				Sh404sefFactory::getPConfig()->windowSizes['editurl']['y'], $top = 0, $left = 0, $onClose = '', $modalTitle, $params);
		}
			?>
          </td>
